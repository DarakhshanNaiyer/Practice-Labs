...
public class BadCharacterHeuristic {
   ...
   //A utility function to get maximum of two integers
    ...
    static void badCharHeuristic( char []str, int size,int badchar[])
    {
     ...
     // of a character (indexes of table are ascii and values are index of occurence)
    ...
    static void search( char txt[],  char pat[])
    {
     ...
     int s = 0;  // s is shift of the pattern with respect to text there are n-m+1 potential allignments
     ...
     while(s <= (n - m))
     {
         ...
         if (j < 0)
         {
             ...
             /* Shift the pattern so that the next character in text aligns with the last
                occurrence of it in pattern.The condition s+m < n is necessary for the case when pattern occurs at the end of text */
